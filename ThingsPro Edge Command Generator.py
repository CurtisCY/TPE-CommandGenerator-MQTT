            ...
            if(len(varAction.get())!=0):
                if(len(var_key_1.get())==0 and len(var_key_2.get())==0 and len(var_key_3.get())==0 and len(var_key_4.get())==0 \
                    ...
                    payload = {
                        'path':f'{varAction.get()}',
                        'method':f'{request_method}',
                        'headers':[{'request-expired-time':f'{varTime.get()}'},{'request-id':f'{varId.get()}'}],
...
request_action = ('Get device IP','Get device general info','Get cellular status','Get DLM cloud status','Get app status'\
...
varId = tk.Spinbox(window, from_= 1, to = 20)
